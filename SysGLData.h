...
typedef struct arc_100  //圆弧实体
{
    ...
    float X2,Y2;       //圆弧起点坐标
    float X3,Y3;       //圆弧终点坐标
    float angle;//圆弧对应的角度  弧度制  IGES格式中没有这个变量，这里是为了方便转126添加的变量
    float Radius;//圆弧的半径   IGES格式中没有这个变量，这里是为了方便转126添加的变量
...
{
    int typeNum;        //实体类型号
    int K1_num;         //B样条曲线1 次数
    int K2_num;         //B样条曲线2 次数
    int M1_num;         //基函数1 次数
    int M2_num;         //基函数2 次数
...
typedef struct para_surface_142//参数曲面上的曲线，类型为142
{
    ...
    int cptr;           //指向外边界曲线C在D区的位置,(曲线C是指曲线B映射到欧几里德三维空间后的曲线，一般很少用)
...
typedef struct trimmed_surface_144//裁剪曲面，类型为144
{
    ...
    line_type generatrix;//如果是旋转曲面,获取母线
    float start_angle;
    float end_angle;
    float angle;        //折弯面的折弯角度
    int pt_surface;     //指向曲面实体在D（索引）区的位置   实体目录的行号
    ...
    void *pCurves[30][20];  //指向边界曲线的二维指针数组   裁剪曲线
    int curveType[30][20];  //边界线的曲线实体类型
    int curveCount[30];     //边界线的曲线的个数

    //此处添加曲面的轮廓线
    void *pOutlines[30][20];//外轮廓线
